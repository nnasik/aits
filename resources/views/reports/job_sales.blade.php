...
<html>
<head>
    <title>AITS Job Report - {{auth::user()->name}}</title>
...
<div class="container my-4">
    ...
    <h4 class="mb-3">AITS Job Report for {{auth::user()->name}}</h4>
    ...
    <div class="card-body">
        <div class="d-flex flex-wrap gap-2">
            ...
            <select id="filter_job_status" class="form-select w-auto">
                ...
                @foreach($workOrders->pluck('status')->unique()->filter() as $s)
            ...
            <select id="filter_payment_status" class="form-select w-auto">
                ...
                @foreach($workOrders->pluck('payment_status')->unique()->filter() as $ps)
    ...
    <table id="reportTable" class="table table-striped table-bordered">
        ...
        <tbody>
            ...
            <tr>
                <td>{{ $wo->id }}</td>
                <td>{{ $wo->date }}</td>
                <td>{{ $wo->company_name_in_work_order }}</td>
                <td>{{ $wo->status }}</td>
                <td>{{ $wo->training_status }}</td>
                <td>{{ $wo->certificate_status }}</td>
                <td>{{ $wo->invoice_no }}</td>
                <td>{{ $wo->invoice_date }}</td>
                <td>{{ number_format($wo->invoice_amount, 2) }}</td>
                <td>{{ $wo->invoice_due_date }}</td>
                <td>{{ $wo->payment_status }}</td>
            </tr>
            @endforeach
        </tbody>
    </table>
